...
function twentytwenty_sidebar_registration() {
	...
	register_sidebar(
		array_merge(
			$shared_args,
			array(
				'name'        => __( 'Shop Sidebar', 'twentytwenty' ),
				'id'          => 'sidebar-1',
...
function woocommerce_header_add_to_cart_fragment( $fragments ) {
    ...
    ?>
    <span class="counter"><?php echo $woocommerce->cart->cart_contents_count; ?></span>
    <?php
...
class Custom_Walker_Nav_Menu extends Walker_Nav_Menu {
    ...
    function start_lvl( &$output, $depth = 0, $args = array() ) {
        ...
        // Add a div for the dropdown and col only for the first level
        if ( $depth == 0 ) {
            $output .= "\n$indent<div class=\"dropdown\">\n";
        }

        // Create a new column for each level of child items
        $output .= "$indent<div class=\"col\"><ul>\n";
    }
    ...
    function end_lvl( &$output, $depth = 0, $args = array() ) {
        ...
        // Close the divs for the columns and dropdown
        if ( $depth == 0 ) {
            $output .= "$indent</ul></div></div>\n"; // Close the last col and dropdown
        } else {
            $output .= "$indent</ul>\n"; // Just close the ul for sub-menus
        }
    }
    ...
    function start_el( &$output, $item, $depth = 0, $args = array(), $id = 0 ) {
        ...
        // Get child menu items for this menu item
        ...
        // Add mega-menu class if there are multiple child items at the top level
        if ( $depth === 0 && count( $children ) > 2 ) {
            $classes[] = 'mega-menu'; // Add mega-menu class
        }
        ...
        // Add strong tag for menu titles at the top level
        if ( $depth === 1 && count( $children ) > 1 ) {
            $item_output .= '<strong class="nav-title">' . apply_filters( 'the_title', $item->title, $item->ID ) . '</strong>';
        } else {
...
function custom_theme_footer_logo_customizer( $wp_customize ) {
    ...
    // Add control for footer logo upload
    $wp_customize->add_control( new WP_Customize_Image_Control( $wp_customize, 'footer_logo_control', array(
        'label'      => __( 'Upload Footer Logo', 'yourtheme' ),
        'section'    => 'footer_logo_section',
        'settings'   => 'footer_logo',
        'description'=> 'Upload a logo to appear in the footer',
    )));
}
add_action( 'customize_register', 'custom_theme_footer_logo_customizer' );
...
function add_content_after_addtocart() {
    ...
    if( $product->is_type( 'simple') ){
        echo '<a href="'.$checkout_url.'?add-to-cart='.$current_product_id.'" class="buy-now button">Buy it Now</a>';
